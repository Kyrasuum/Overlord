...
if not isNull _dsp then
{
   ...
   _ctrlRestart ctrlSetText format["Press %1 to close this menu", getText(_cfgPath >> "restart")];
   _ctrlHostedBy = _dsp displayCtrl 5;
   _ctrlHostedBy ctrlSetText format["%1", getText(_cfgPath >> "hostedBy")];
   _ctrlantiHACK = _dsp displayCtrl 6;
   _ctrlantiHACK ctrlSetText format["Created by: %1", getText(_cfgPath >> "createdBy")];
